...
export async function resetDb(): Promise<void> {
  await db.execute(
    sql`TRUNCATE TABLE near_duplicate_pairs, photo_embeddings, ai_analysis_events, photo_collections, collection_negative_photos, project_photos, projects, collections, ratings, photo_attribution_tags, attribution_tags, photos, albums, "user", session, account, verification, users RESTART IDENTITY CASCADE`,
  );
}
